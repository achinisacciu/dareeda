...
def test_ml_exploratory():
    ...
    res3 = ml_exploratory.run(
        df, df_full, semantic_types, groups,
        context={"target": "inesistente", "problem_type": "regression"}
    )
    print(res3)
    assert "error" in res3

    print("\n[Caso 4] Target tutta null:")
    res4 = ml_exploratory.run(
        df, df_full, semantic_types, groups,
        context={"target": "target_null", "problem_type": "regression"}
    )
    print(res4)
    assert "error" in res4

    print("\n[Caso 1] Target valida:")
    res1 = ml_exploratory.run(
        df, df_full, semantic_types, groups,
        context={"target": "target_valid", "problem_type": "classification"}
    )
