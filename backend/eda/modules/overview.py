...
def _generate_notes(n_rows, n_cols, pct_missing, groups) -> list[str]:
    ...
    if pct_missing > 20:
        notes.append(
            f"Dataset con alta percentuale di valori mancanti ({pct_missing:.1f}%). Valutare la qualità delle sorgenti."
        )
    n_id = len(groups.get("id", []))
    if n_id > n_cols * 0.3:
        notes.append(
            f"Presenti {n_id} colonne ID o quasi-ID ({round(n_id / n_cols * 100)}% del totale). "
            "Probabilmente da escludere."
        )
    n_cat = len(groups.get("categorical_nominal", [])) + len(groups.get("categorical_ordinal", []))
    if n_cat > n_cols * 0.5:
        notes.append(
            f"Dataset prevalentemente categorico ({n_cat}/{n_cols} colonne). Encoding necessario per la modellazione."
        )
    n_num = len(groups.get("numeric_continuous", [])) + len(groups.get("numeric_discrete", []))
    if n_num == 0:
        notes.append(
            "Nessuna colonna numerica rilevata. Verificare numerici memorizzati come stringhe."
        )
    if n_rows > 500_000:
        notes.append(
            f"Dataset di grandi dimensioni ({n_rows:,} righe). Analisi eseguita su campione rappresentativo."
        )
