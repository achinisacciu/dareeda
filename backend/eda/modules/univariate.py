...
def _comment_numeric(skew, kurt, n_out, n, norm_flag, cv) -> str:
    ...
    if abs(skew) < 0.5:
        ...
        parts.append(
            f"La distribuzione presenta asimmetria positiva (skewness={skew:.2f}): coda destra pronunciata."
        )
    else:
        parts.append(
            f"La distribuzione presenta asimmetria negativa (skewness={skew:.2f}): coda sinistra pronunciata."
        )
    if kurt > 3:
        parts.append("Code pesanti rilevate (leptocurtica): probabile presenza di outlier estremi.")
    if n_out > 0:
        parts.append(
            f"Rilevati {n_out} outlier ({round(n_out / n * 100, 1)}%) secondo il metodo IQR."
        )
...
def _suggest_numeric(skew, n_out, n, cv) -> list[dict]:
    ...
    if abs(skew) > 1:
        suggestions.append({
            "motivo": f"Skewness elevata ({skew:.2f})",
            "trasformazione": "Log o Box-Cox",
            "priorita": "alta",
        })
    if n_out > n * 0.05:
        suggestions.append({
            "motivo": "Outlier >5% dei dati",
            "trasformazione": "Winsorization (P1-P99)",
            "priorita": "alta",
        })
    if cv is not None and cv > 1:
        suggestions.append({
            "motivo": "Alta variabilità relativa (CV>1)",
            "trasformazione": "StandardScaler o RobustScaler",
            "priorita": "media",
        })
    return suggestions
...
def _numeric_discrete(s: pl.Series, col: str) -> dict:
    ...
    freqs = [
        {
            ...
            "pct": round(float(vc["count"][i]) / n * 100, 2),
        }
    ...
    fig = go.Figure(
        go.Bar(
            x=[f["level"] for f in freqs],
            y=[f["count"] for f in freqs],
            marker_color="#1B272F",
            text=[f"{f['pct']}%" for f in freqs],
    ...
    return {
        ...
        "charts": {"bar": _fig(fig)},
        "ai_comment": (
            f"Variabile discreta con {valid.n_unique()} livelli distinti. "
            f"Valore dominante: {mode_val} ({imbalance:.1f}%)."
        ),
    }
    ...
    freqs = [
        {
            ...
            "count": counts_top[i],
            "pct": pcts_top[i],
            "cum_pct": round(sum(pcts_top[: i + 1]), 2),
        }
...
def _comment_categorical(n_unq, card_r, imbalance, n_rare) -> str:
    ...
    if card_r > 90:
        parts.append(
            f"Alta cardinalità ({card_r:.1f}%): questa colonna potrebbe essere un quasi-ID."
        )
    elif card_r > 50:
        parts.append(
            f"Cardinalità elevata ({n_unq} categorie). Valutare raggruppamento o encoding a cardinalità ridotta."
        )
    else:
        parts.append(f"Variabile categorica con {n_unq} categorie distinte.")
    if imbalance > 10:
        parts.append(
            f"Forte sbilanciamento tra le classi (ratio {imbalance:.1f}x): la classe dominante è molto più frequente."
        )
    if n_rare > 0:
        parts.append(
            f"Presenti {n_rare} categorie rare (<1% dei dati). Valutare accorpamento in categoria 'Altro'."
        )
    return " ".join(parts)
...
def _boolean(s: pl.Series, col: str) -> dict:
    ...
    imbalance = (
        round(max(pct_true, pct_false) / min(pct_true, pct_false), 2)
        if min(pct_true, pct_false) > 0
        else None
    )
...
def _datetime(s: pl.Series, col: str) -> dict:
    ...
    try:
        ...
        range_days = (
            (dt_valid.max() - dt_valid.min()).days
            if hasattr((dt_valid.max() - dt_valid.min()), "days")
            else None
        )
        ...
        return {
            ...
            "stats": {
                ...
                "n_unique": int(dt_valid.n_unique()),
            },
            "charts": {"by_year": _fig(fig_year), "by_month": _fig(fig_month)},
            "ai_comment": (
                f"Variabile temporale con range da {min_dt} a {max_dt} "
                f"({range_days} giorni). Verificare gap temporali e stagionalità."
            ),
        }
    except Exception as e:
...
def _text(s: pl.Series, col: str) -> dict:
    ...
    return {
        ...
        "ai_comment": (
            f"Colonna testuale. Lunghezza media {avg_len:.0f} caratteri, P95 {p95_len:.0f}. "
            "Richiedere pipeline NLP dedicata per analisi semantica."
        ),
    }
...
def _id_col(s: pl.Series, col: str) -> dict:
    ...
    return {
        ...
        "stats": {
            "count": n,
            "missing_pct": round(s.null_count() / n * 100, 2),
            "n_unique": n_unique,
            "unique_ratio": uniq_r,
            "duplicate_count": n_dup,
        },
        "ai_comment": (
            f"Colonna identificativa ({uniq_r:.1f}% valori unici). "
            "Escludere dall'analisi statistica e dalla modellazione."
        ),
        "action": "escludere",
    }
    ...
    try:
        ...
        return {
            ...
            "stats": {
                "count": n,
                "missing_pct": round(s.null_count() / len(s) * 100, 2),
                "min": round(float(num.min()), 5),
                "max": round(float(num.max()), 5),
                "n_unique": int(num.n_unique()),
                "out_of_range_count": int(((num < -180) | (num > 180)).sum()),
            },
            "ai_comment": (
                f"Colonna geografica. Range [{float(num.min()):.4f}, {float(num.max()):.4f}]. "
                "Verificare coordinate fuori range."
            ),
        }
    except Exception:
